#!/usr/bin/env python3
"""
Sales by Product/Service Pipeline -- Gmail to Database
...
Dedup check: skips emails whose received date already exists in raw_sales_detail
...
def get_existing_received_timestamps(db) -> set:
    """Get all email received timestamps already loaded into raw_sales_detail.
    Uses full timestamp (not just date) so multiple emails on the same day are each loaded.
    """
    rows = db.fetch(
        f'SELECT DISTINCT email_received_date FROM {SCHEMA_RAW}.raw_sales_detail '
...
def start_pipeline_run(db, run_id: str, metadata: dict = None):
    ...
    retry_db(
        lambda: db.execute(
            ...
            f'VALUES ($1, $2, $3, $4, $5)',
            run_id, "sales_detail_extract", "running", datetime.now(timezone.utc), metadata
        ),
...
def load_raw_batch(db, rows: list):
    ...
    retry_db(
        lambda: db.executemany(
            f'INSERT INTO {SCHEMA_RAW}.raw_sales_detail (run_id, as_of_date, email_received_date, source_file, data) '
            f'VALUES ($1, $2, $3, $4, $5)',
            tuples
        ),
...
def run_sales_pipeline(max_emails: int = 100, reprocess: bool = False):
    ...
    if existing_timestamps:
        logger.info(f"Found {len(existing_timestamps)} existing received timestamps in raw_sales_detail")
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        for details in message_details:
            ...
            try:
                ...
                transform_sales_for_run(db, run_id)
...
if __name__ == "__main__":
    ...
    run_sales_pipeline(max_emails=args.max_emails, reprocess=args.reprocess)
