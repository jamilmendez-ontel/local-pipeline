-- 138: add `cohort_approved` to analytics.approver_scorecard = approved reports
-- whose SUBMITTED date is in the range (the same cohort dimension as `pending`).
-- Lets the UI show an approval-completion rate for the period: cohort_approved /
-- (cohort_approved + pending) = "of reports submitted in this range, how many are
-- approved vs still waiting". Return type gains a column, so DROP + CREATE.
...
AS $$
  WITH scoped AS (
    SELECT
      ...
      (a.submitted_on_et IS NOT NULL
        AND (p_from IS NULL OR a.submitted_on_et::date >= p_from)
        AND (p_to   IS NULL OR a.submitted_on_et::date <= p_to)) AS submitted_in_window
  ...
  agg AS (
    SELECT
      ...
      count(DISTINCT emp_id) FILTER (WHERE pending_in_window)                          AS pending_employees,
      count(*) FILTER (WHERE submitted_in_window AND task_status = 'approved')         AS cohort_approved,
