-- 083_quote_service_rate_override.sql
-- Allow editing the Service Rate ($) per entry. Adds
-- stg_quote_overrides.service_rate_override (text); v_quote_review's inv_service_rate
-- now = COALESCE(override, chosen/best line rate), with service_rate_overridden flag.
-- (Only the inv_service_rate expression + one flag changed vs 081; rest identical.)

ALTER TABLE data_staging.stg_quote_overrides ADD COLUMN IF NOT EXISTS service_rate_override text;
...
WITH base AS (
  SELECT
    ...
    COALESCE(o.service_rate_override, CASE WHEN opt.line_key IS NOT NULL THEN opt.service_rate ELSE m.inv_service_rate END) AS inv_service_rate,
