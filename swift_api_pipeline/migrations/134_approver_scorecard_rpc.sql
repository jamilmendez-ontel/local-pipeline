-- 134: analytics.approver_scorecard(p_from, p_to) — group-grain daily-report approval
-- turnaround for the HR approver scorecard. One row per approver group (via
-- reference.ref_approver_group). `pending` and `employees` are CURRENT snapshots;
-- approved / on-time / amber / late / avg respect the date window, filtered on the
-- APPROVAL date (approved_on_et). p_from/p_to NULL => all time.
...
AS $$
  ...
  agg AS (
    SELECT
      group_label,
      count(*) FILTER (WHERE is_awaiting_approval)                                     AS pending,
...
VALUES
  ('analytics','approver_scorecard',
   'Function(p_from,p_to): group-grain daily-report approval turnaround. One row per approver group with current pending + employees and windowed approved/on-time/amber/late/avg latency (filtered on approval date). Params NULL = all time.',
   'Powers the HR approver scorecard: which approver group is backed up or slow. Keyed on the group, not the individual, because large groups have several approvers.',
