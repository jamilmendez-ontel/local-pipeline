-- =============================================================================
-- 259_rebuild_timer_clean_advisory_lock.sql
-- Follow-up to 257 (same day, pre-merge review finding): the old
-- DELETE-all body serialized overlapping rebuild_timer_clean() calls as a
-- side effect (every row locked); the change-only sync does not, so two
-- calls racing on a brand-new correction/addition could both insert it.
-- Adds `pg_advisory_xact_lock(hashtext('data_staging.rebuild_timer_clean'))`
-- as the first statement. Everything else is 257's body verbatim
-- (pg_get_functiondef md5 da629223251ab285c68fc8a08779cee5, 13,900 chars).
-- Callers that used to overlap now queue for ~11 s; readers are unaffected.
-- =============================================================================
...
BEGIN
  ...
  IF position('pg_advisory_xact_lock' IN def) > 0 THEN
    RAISE EXCEPTION '259: already applied';
  END IF;
  IF md5(def) <> 'da629223251ab285c68fc8a08779cee5' THEN
    RAISE EXCEPTION '259: live rebuild_timer_clean is not the 257 body (md5 %); re-capture before applying', md5(def);
  END IF;
END $$;
