-- 081_quote_review_phrase_match.sql
-- Improve the directory match: the directory combines scope parts the path SEPARATES
-- (directory Project 'FTTH_Phase 1' vs asset_id '.../FTTH/Phase 1/...'). Segment-array
-- containment (080) couldn't match those. Switch to PHRASE containment: flatten the
-- path to one string treating / _ - and whitespace ALL as a single space, then require
-- each directory token (GC/Carrier/Market/Project, normalized) to appear as a
-- space-bounded phrase. Token source still includes the effective (override-aware)
-- category values. Verified 2026-06-09: matched 211 -> 233 (222 ready / 11 conflict /
-- 89 missing). API read stays well under timeout.
--
-- (Only the directory-join tail changed vs 080; base CTE identical.)
...
LEFT JOIN LATERAL (
  ...
  WHERE k.gc_n <> '' AND k.carrier_n <> '' AND k.market_n <> '' AND k.project_n <> ''
    ...
    AND position(' '||k.project_n||' ' IN seg.pathstr) > 0
