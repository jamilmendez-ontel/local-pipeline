-- 137: add `pending_employees` (distinct employees with a report awaiting approval)
-- to analytics.approver_scorecard, using the SAME submitted-date window as `pending`
-- so it tracks the Pending column. Return type gains a column, so DROP + CREATE.
