-- 119_timer_clean_fill_user_name_ROLLBACK.sql
-- Restores rebuild_timer_clean() to its pre-119 form (correction-materialized
-- rows inserted WITHOUT user_name / user_role). Does NOT un-backfill the
-- one-time UPDATE; that data is harmless to keep. To fully revert the data,
-- the next rebuild will simply re-null those names.
