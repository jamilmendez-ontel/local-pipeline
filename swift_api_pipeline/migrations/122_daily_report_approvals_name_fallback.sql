-- 122: employee_name falls back to the timer attendance user_name when the
-- employee is not (yet) in reference.ref_employees. Superseded by 123 (which adds
-- the more complete asset-name parse) but kept for history; applied to prod via MCP 2026-06-25.
CREATE OR REPLACE VIEW analytics.v_daily_report_approvals AS
 ...
 SELECT t.emp_id,
    COALESCE(e.full_name, tmr.user_name) AS employee_name,
