-- 080_quote_review_directory_contains.sql
-- Switch the directory match from exact composite to TOKEN-CONTAINMENT (the
-- legacy-sheet model): a directory key applies when the entry's normalized token
-- set contains its GC, Carrier, Market AND Project tokens. Token set = normalized
-- asset_id path segments PLUS the entry's effective (override-aware) category
-- values (so manual fixes count). Most-specific key wins (longest combined tokens).
-- Adds directory_conflict (true when the matched key has conflicting recipients).
-- Verified 2026-06-09: 322 rows, 211 matched, 11 hit a conflicted key. API read ~1.3s.
...
FROM base
LEFT JOIN LATERAL (
  SELECT (SELECT array_agg(analytics.quote_norm(s)) FROM unnest(string_to_array(base.asset_id,'/')) s)
         || ARRAY[analytics.quote_norm(base.gc), analytics.quote_norm(base.carrier),
                  analytics.quote_norm(base.market), analytics.quote_norm(base.project)] AS arr
) seg ON true
LEFT JOIN LATERAL (
  SELECT k.recipient, k.cc, k.has_conflict, 1 AS hit
  FROM analytics.v_quote_directory_keys k
  WHERE k.gc_n <> '' AND k.carrier_n <> '' AND k.market_n <> '' AND k.project_n <> ''
    AND k.gc_n = ANY(seg.arr) AND k.carrier_n = ANY(seg.arr)
    AND k.market_n = ANY(seg.arr) AND k.project_n = ANY(seg.arr)
  ORDER BY length(k.gc_n || k.carrier_n || k.market_n || k.project_n) DESC
  LIMIT 1
) dir ON true;
