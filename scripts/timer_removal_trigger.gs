 ...
 *
 * Includes a 5-second cooldown to prevent duplicate dispatches from
 * Google Forms double-fire on submit.
 *
 * Setup:
...
function onFormSubmit(e) {
  ...
  var lastFire = cache.get('last_removal_dispatch');
  if (lastFire) {
    Logger.log('Skipping duplicate dispatch (cooldown active)');
    return;
  }
  cache.put('last_removal_dispatch', 'true', 5);  // 5-second cooldown
