 ...
 *
 * Batches Remove Timer Entry form submissions into a single repository_dispatch
 * (`timer-correction-apply`) to GitHub Actions. The same apply workflow reads
 * the removal, stores it in app_timer.entry_removals, and rebuilds
 * stg_timer_activities_clean.
 *
 ...
 *     race or double-fire.
 *   - pollPendingDispatch() is the 5-minute poller. It is installed in the
 *     CORRECTION project only; it lives here too so either project can host
 *     it, and the D1 lock makes an accidental second poller harmless.
 *
 ...
 *   4. COORDINATION_SHEET_ID below must be IDENTICAL in timer_correction_trigger.gs
 ...
 *        - do NOT install pollPendingDispatch here (correction project hosts it)
 *        - DELETE any leftover firePendingRemovalDispatch trigger
 *   6. Run onFormSubmit once manually to grant the Drive scope
 */
...
// Only the project with POLLER_HOST = true actually fires dispatches. That is
// the CORRECTION project. Here it is false, so an accidental poller install in
// this project is a no-op. Flip it only if the poller is deliberately moved.
var POLLER_HOST = false;

var LOG_LABEL = 'Removal';
...
function _pollPendingDispatchLocked() {
  ...
  if (success) {
    ...
    Logger.log('Removal apply workflow triggered (batched window from ' + startedAt + ')');
    return;
  }
