...
**Post-merge runner test + fix PR (07:00–07:15 ET):** the first dispatched dry run on GitHub failed loudly with HTTP 403 from officialgazette.gov.ph (local runs got 200). `--probe` from the runner: Cloudflare in front of gov.ph (Official Gazette AND pco.gov.ph) returns 403 to GitHub runner IPs with any User-Agent; lawphil.net (nginx) and Nager.Date are reachable. Fix: lawphil's per-year proclamation index (number + signing date + full title, cp1252; lags the Gazette ~2 weeks) is now the PRIMARY source, scanned whole every run (plus last year's page in Jan-Feb); the Gazette walk is best-effort (403 = warning). Items merge by key. Locality scope regex widened ("in Mountain Province", lawphil's "Municpality" typo, "... DAY IN <place>"). 28 tests. Runner dry run with real DB: lawphil 249, Gazette blocked, 0 findings + the Nager 2026-10-31 verify item, run row written. Dry/error rows purged; the seed coverage row stays.

**Session close (07:20 ET):** both PRs merged (#52 `a79a7c6`, #53 `c553cd4`); main dry run on the runner passes; `pipeline.holiday_watch_runs` holds only the seed coverage row (all dry/error test rows purged; nothing was ever emailed). Expected behaviour from here: the first scheduled run (Thu 22:00 UTC cron; Mon 6 PM ET once the Apps Script trigger exists) emails Jamil ONE "VERIFY" item, 2026-10-31 All Saints' Day Eve (Nager lists it, Proclamation 1006 did not declare it), reported once and never repeated; no reply is needed and nothing reads replies. After that, silent until a real proclamation shows up.
Open: (1) Jamil pastes whole `scripts/pipeline_trigger.gs` into the nanoninth Apps Script project and runs `setupHolidayFeedWatchTrigger()` once; (2) nothing consumes `ref_holidays` yet (`fn_approval_deadline` still ignores PH holidays, `fn_workdays_scrub` still reads `ref_scrub_holidays`); (3) local `.env` still points at the IPv6 direct DB host, local runs need the pooler env overrides.

**07:25 ET:** Jamil pasted `pipeline_trigger.gs` and ran `setupHolidayFeedWatchTrigger()`. First live fire: Mon 2026-08-31 6 PM ET (Tue Sep 1 6 AM PHT); Jamil confirms the one expected VERIFY email then.

## 2026-08-28 (4:30 AM - 5:00 AM ET) - timer: split member emails into a second daily run (pipeline-timer-emails, ~6 AM ET / 18:00 PHT)
- Why: a Swift stop can reach the report data hours late (3 cases in Aug 2026: Abbie 08-27 18:13 ET stop NULL at the 01:19 ET extract, visible by 03:29; Ton 08-12; Abbie 08-10). With extract and --send 9 minutes apart the daily email flagged them "still running" and hours were short until the next night. Members cannot edit timers in Swift, so this is sync latency, not user action.
- `pipeline-timer.yml` (~1:15 AM ET): removed --remind / --send / --resend. Still extract + backfill + raw & clean Excel exports (Drive + email; Sheena's report needs the 13:15 PHT clean export) + --apply + MV refresh + clean-export alert.
- NEW `pipeline-timer-emails.yml` (~6:00 AM ET): dispatch type `pipeline-timer-emails`, same `pipeline-timer` concurrency group; extract (`--email-on-failure-only`) -> backfill -> --apply (hard-fails the job so no email goes out on stale data) -> MV refresh -> --remind -> --send -> --resend. No exports. No retry around the send steps (--send upserts daily_notifications and would re-send).
- `scripts/pipeline_trigger.gs`: `triggerTimerEmails()` + `setupTimerEmailsTrigger()` (atHour 6 ET, nearMinute 0; PHT arrival drifts 18:00/19:00 with US DST, accepted). Excluded from testAllDispatches. Jamil must paste the whole file and run setupTimerEmailsTrigger() once after merge.
...
### 2026-09-17 09:55-10:50 ET — Timer duplicates: only the member removes entries (fix/duplicate-member-removals-only)
- Trigger: Nath's 2026-09-15 confirmation email showed his real 08:05-09:32 General Admin session as `DUPLICATE - NOT COUNTED` after he removed only the two 8h07m runaway snapshots. Root cause: overlap clustering is transitive, so the two runaways (08:05-16:12) bridged the 08:05 session and the separate 14:43-15:13 session into one group `b614c1665bf9`; `_resolve_duplicate_for_action` then collapsed the group to ONE survivor (shortest alive = the 30-min entry) and wrote an `auto_resolved_sibling` removal for the 1h26m entry although the two never overlapped. The DUPLICATE badge on the 14:43 entry was correct (it sits inside the runaway) and was never the bug.
- Rule decided by Jamil: the system removes nothing; a member's Remove/Edit touches exactly the entry acted on. Rewrote `_resolve_duplicate_for_action`: no `entry_removals` writes, no survivor pick; re-cluster the alive copies by overlap; 2+ alive copies still overlapping keep the group open (reminders continue) with `entries` narrowed to those copies (so the provisional latest-end rule in `rebuild_timer_clean()` hides only real leftovers); otherwise resolve with `rejected_entries = []`, `resolved_by = 'member'`. Deleted `_pick_shortest` and `_fallback_after_survivor_removal` (legacy resolved groups keep their state; removing their survivor is now just a member removal).
- Tests: `tests/test_duplicate_survivor.py` replaced by `tests/test_duplicate_member_removals.py` (15 cases: nath-shaped mixed cluster step by step, same-start drift copies, edit paths, narrowed snapshot, still-running copies, legacy resolved group). 23 pass with the confirmation-email tests. Pre-existing: 6 asset-tasks resilience failures on main (stale monkeypatch vs `estimate=`), untouched.
- Audit of all 32 live `auto_resolved_sibling` removals: 26 overlap a surviving clean row (true copies, left alone), 2 inert (raw row gone), 4 were real sessions overlapping nothing: lenard 08-10 111.5 min, madel 08-24 75.1 min, steph.familara 09-09 7.2 min, nath.parajas 09-15 86.8 min. Migration 267 (applied live, `rebuild_timer_clean()` run) reverts those 4 and clears `rejected_entries` on their groups; all 4 verified back in `stg_timer_activities_clean`; Nath's 09-15 day now totals 448.6 min.
- Still system-driven and deliberately unchanged (flagged to Jamil): `auto_resolve_stale` (7-day-old open groups keep the longest copy via `rejected_entries`) and `_reconcile_resolved_group_stragglers` (late snapshots on a group resolved by Edit, latest end wins). Edit no longer sets aside siblings either; the member removes them.
- premerge-review Lane A (code reviewer) finding, fixed before merge: an Edit on a copy with alive overlapping siblings left the edited entry in the narrowed snapshot, so the rebuild's provisional latest-end rule could hide its raw row while the corrections path inserted a synthetic corrected row (transient double count). Now the acted-on entry leaves the open snapshot for Edit as well as Remove; the correction applies to its raw row in place. Lanes: call-site ordering, JSONB codec params, narrowed snapshots vs `run_remind`/`auto_resolve_stale`, legacy entries without `start_time`, migration 267 idempotency all clean. 24 tests pass.
- Shipped: squash-merged #79 as `ce46bbc` (branch deleted), migration 267 live with `rebuild_timer_clean()` run. OnPoint: logged as INIT-759 (done, 2026-09-17) under INIT-043 Timer Activities System via `onpoint/out/push_timer_member_removals_subtask.py`. Memory + Obsidian `local-pipeline.md` updated. Open for Jamil: tell Nath his 09-15 hours are restored (his confirmation email still shows the old badge); decide whether `auto_resolve_stale` (keeps LONGEST after 7 days) should also become member-only.
- Closed by Jamil (11:20 ET): Nath told his hours are restored. DECISION: `auto_resolve_stale` (7-day stale groups keep the longest copy) stays as is, no work planned.

### 2026-09-23 01:25-03:10 ET — task_clean keeps the revision number (fix/task-clean-keep-revision-suffix, mig 268)
- Trigger: Jamil asked to confirm the condition behind `task_clean`, then changed it: strip only the leading sequence prefix, stop stripping the trailing revision number. Audit of the existing column first turned up three defects: all of calendar 2025 had `task_clean IS NULL` (119,761 rows in `stg_timer_activities_clean`, originating upstream in `stg_timer_activities` — the historical backfill loaded `task` and never computed the clean name); 2,782 rows still carried lettered prefixes (`3F. COP Upload Complete`) because migration 013 used `'^\d+\.\s*'` with no `[a-zA-Z]?` and no repeat; and `rebuild_timer_clean()`'s corrections branch re-derived the value with a weaker regex (`'^\d+\.\s+'`) than every other branch.
- WHY the rule change matters: `task_clean` is the revenue join key (`mv_timer_revenue` joins `ref_task_revenue_rates.task_name_norm = task_clean`), and the rate sheet prices revisions BELOW first passes (AT&T Live Review Complete $200 vs `Live Review Complete 2` $83; COP Revision Complete $30 / `2` $20 / `3` $10). Stripping the suffix made 24 of 60 rate keys unreachable and billed every revision hour at the full first-pass rate. Reachable keys 36 -> 54 of 60.
- New rule, byte-identical in Python and SQL: `BTRIM(regexp_replace(t, '^([0-9]+[a-zA-Z]?\.[[:space:]]*)+', ''), E' \t\n\r\f\v')`. Code: `clean_task_name()` + the four SQL expressions in `transform.py` (user priorities, asset tasks, QA forms, GC asset tasks), the fallback join key in `timer_correction_review.py:856` (a 6th copy of the old weak regex, missed by the original scope), `gc-asset-lake/transform.py`, `ontel-data-platform/asset_fields.py`.
- Whitespace parity was a premerge finding, not cosmetic: SQL used `' *'` (ASCII space) while Python used `\s` / bare `.strip()`, which also match NBSP. For a join key, one raw name reaching two pipelines had to clean identically. Hardened both sides and PROVED the change a no-op before committing: 0 of 4,486,170 rows differ between the simple and hardened expressions. Cross-repo parity test covers tab, NBSP, `4.NoSpace`, `1. 2a. `, `12.`, empty, None.
- Migration 268 applied 02:45-03:05 ET, batched with COMMIT every 50k rows (1.11M rows across 8 tables; a single transaction was rejected as unsafe on a Small-tier instance with a standing Disk IO budget concern). Every backfill count matched the preflight estimate to the row: asset_tasks 479,291 (253.6s), asset_tasks_inc 215,728, qa_form 147,994, timer_activities 129,920, timer_clean 129,298, gc_tracker 6,214, user_priorities 581, entry_additions 5, asset_tasks_gc 0. Assertion covers all 9 tables and passes.
- GOTCHA (cost one failed apply): the first run aborted at the assertion because I had squash-merged to `main` LOCALLY but not pushed. The 5-minute `user_priorities_extract` runs from GitHub `main`, so it was still executing pre-merge code and rewrote 581 rows with the old rule mid-migration. Pushed `5bb3798`, re-ran the backfill + assertion + the VACUUM/ANALYZE section that never executed. PUSH BEFORE APPLYING. Deploy then verified properly: the 02:43 ET run wrote the new rule and 0 rows disagree.
- `VACUUM (ANALYZE)` was a preflight finding worth keeping: autovacuum would never have fired on `stg_asset_tasks` (threshold 567,645 dead tuples, backfill makes 479,291) and has never processed `stg_asset_tasks_inc` at all, while 110,607 rows leave the `COP Revision Complete` bucket and six analytics objects do equality filters on that column.
- Verified after refresh: timer_clean NULLs 119,761 -> 0; prefixed 2,782 -> 0; approved-LR asset set 18,484 -> 18,483 (exactly the one asset predicted); `agent.schema_metadata` rows stating the old rule 4 -> 0. `mv_timer_revenue` 133,943 -> 208,275 rows, priced 192,456 / $12,721,122.96 — the +74,332 is dominated by 2025 (71,145 rows, $4,320,793.13) which could never be priced while its `task_clean` was NULL; that revenue was always earned, just unattributable. The rule change itself re-rates 7,725 rows across 72 revision labels, $157,168.99.
- Nearly shipped a data-loss bug: section 4 originally told the operator to refresh `analytics.mv_daily_completion_gc`, which holds 5,809 rows while its only source `stg_asset_tasks_gc` is EMPTY (confirmed 5809 vs 0). Refreshing would have wiped it. Removed, with the reason in-file. Note `refresh_analytics_gc()` (`transform.py:1790`) will still do exactly that whenever the GC pipeline next runs — pre-existing, unfixed, flagged.
