...
| `pipeline-asset-tasks.yml` | Nightly Apps Script + 6 PM PHT chain | Heavy (~40 min): asset_tasks → MVs → end-of-run dispatches. `downstream=all` (nightly 12:01 AM ET: export + date-validator + Fri compliance + Mon invoicing/PO), `validator` (6 PM PHT run fired by gmail-scraper on success: date-validator only), `none` |
